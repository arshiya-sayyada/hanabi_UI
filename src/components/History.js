...
function History() {
  const { moveHistory, isPlayerTurn, gameOver } = React.useContext(BoardContext);
  console.log()
  ...
  //React.useEffect(() => {
  //  scrollToBottom();
  //}, [scrollToBottom, moveHistory.length]);
    ...
    <div className="log-wrapper">
      ...
      {moveHistory.length > 0 ? (
        <div className="log-scroll" ref={logContainerRef}>
          {[...moveHistory].map((log, idx) => (
            <span
              key={idx}
              className={cx("log-row", {
                bright: idx === moveHistory.length - 1
              })}
            >
              {log}
            </span>
